...
const qleverPrefixes = "PREFIX wd: <http://www.wikidata.org/entity/> PREFIX wdt: <http://www.wikidata.org/prop/direct/> PREFIX wikibase: <http://wikiba.se/ontology#> PREFIX schema: <http://schema.org/> PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>";
...
const fallbackQuery = `${qleverPrefixes}
SELECT ?person ?personLabel ?personDescription ?children ?image ?sitelinks ?articleTitle WHERE {
  VALUES ?occupation { wd:Q177220 wd:Q33999 wd:Q937857 wd:Q639669 wd:Q2066131 wd:Q32112 wd:Q2252262 wd:Q947873 wd:Q4610556 }
  ?person wdt:P31 wd:Q5; wdt:P106 ?occupation; wdt:P569 ?birth; wdt:P1971 ?children; wdt:P18 ?image; wikibase:sitelinks ?sitelinks.
  FILTER(YEAR(?birth) >= 1900 && ?children >= 0 && ?children <= 30)
  ?article schema:about ?person; schema:isPartOf <https://en.wikipedia.org/>; schema:name ?articleTitle.
...
ORDER BY DESC(?sitelinks)
LIMIT 500`;
const fallbackResponse = await fetch(`${qlever}?${new URLSearchParams({ query: fallbackQuery, format: "json" })}`, { headers });
if (!fallbackResponse.ok) throw new Error(`Wikidata fallback request failed: ${fallbackResponse.status}`);
bindings.push(...(await fallbackResponse.json()).results.bindings);
...
for (const row of rows) {
  ...
  if (candidates.length === 120 - curated.length) break;
}
if (curated.length + candidates.length < 120) throw new Error(`Only found ${curated.length + candidates.length} usable figures.`);
