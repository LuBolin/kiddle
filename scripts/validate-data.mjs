...
const ids = new Set();
const errors = [];
const localizedFields = ["displayName", "descriptor", "countingRuleSummary", "explanation", "imageAlt"];
...
for (const figure of figures) {
  ...
  if (figure.status === "active" && (!figure.sources?.length || !figure.image?.url || !figure.image?.sourceUrl || !figure.image?.licence || !figure.image?.attribution)) errors.push(`${figure.id}: active record is incomplete`);
  if (figure.status === "active" && ["en", "zh"].some((language) => localizedFields.some((field) => typeof figure.text?.[language]?.[field] !== "string" || !figure.text[language][field].trim()))) errors.push(`${figure.id}: active record needs complete English and Simplified Chinese text`);
