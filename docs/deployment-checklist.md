...
- [x] Migrate every Figure to the canonical single-record localized schema in `data-crawling-guide.md`; remove the legacy top-level English text fields after rendering and validation use `text.<language>` with field-level English fallback.
...
- [x] Extend `npm run validate-data` to reject missing English fallback text and incomplete required Chinese text. Add fact-divergence checks as new localized factual fields are introduced.
...
- [x] Keep one JSON file per category using the canonical Figure contract: shared facts and media once, with reader-facing text under `text.en` and `text.zh`.
